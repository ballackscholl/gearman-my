...
public class GearmanWorkerImpl
    ...
    private static final org.slf4j.Logger LOG_STATISTICS = LoggerFactory.getLogger(
            Constants.GEARMAN_STATISTICS_LOGGER_NAME);
    private String id;
    ...
    public GearmanWorkerImpl(ExecutorService executorService) {
        ...
        id = DESCRIPION_PREFIX + ":" + getHostName() + ":" + Thread.currentThread().getId();
    ...
    private void statistics(){
    	...
    	if(this.statisticsCount >= 4294967296L){
    		this.statisticsCount = 0;
    	}else{
    		this.statisticsCount++;
    	}
    	
    	if(this.statisticsCount % 100 == 0){
    		if(executorService != null && this.executorService instanceof ThreadPoolExecutor){
    			ThreadPoolExecutor executor = (ThreadPoolExecutor)this.executorService;
    			LOG_STATISTICS.info("statistics[thread]:" + executor.toString() + " tasks:" + executor.getTaskCount());
    		}
    		Collection<GearmanJobServerSession> sessions = sessionMap.values();
    		for(GearmanJobServerSession session :sessions){
    			LOG_STATISTICS.info("statistics[task]:" + session.toString() + " ActiveTasks:" + 
    					session.getNumberOfActiveTasks());
    		}
    	}
    }
    
    private boolean isAllowGrabJob(){
    	if(this.executorService != null 
    			&& this.executorService instanceof ThreadPoolExecutor){
    		
    		ThreadPoolExecutor executor = (ThreadPoolExecutor)this.executorService;
    		long queueSize = executor.getQueue().size();
    		long maxPoolSize = executor.getMaximumPoolSize();
    		if(queueSize > maxPoolSize * 2){
    			LOG.warn("!!!!!! queueSize > maxPoolSize * 2");
    			return false;
    		}
    	}
    	return true;
    ...
    public void work() {
        ...
        while (isRunning()) {
            ...
            for (GearmanJobServerSession sess : sessionMap.values()) {
                ...
                if (sessTask == null && isAllowGrabJob()) {
